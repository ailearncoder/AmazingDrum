...
public class PlayDrumActivityBle extends Activity {
        ...
        public void run() {
            switch (bleState) {
                ...
                case DEVICE_STATE_LINKFAILED:
                    cancelProgressDialog();
                    MyUtil.showToast(PlayDrumActivityBle.this, "设备连接失败\n" + bleLink.getErrorMsg());
                    break;
                case DEVICE_STATE_LINKLOST:
                    cancelProgressDialog();
                    MyUtil.showToast(PlayDrumActivityBle.this, "设备连接丢失\n" + bleLink.getErrorMsg());
                    break;
            }
            isUIRun = false;
        }
        ...
        public void OnDeviceStateChanged(BleLink bleLink, BleLink.DeviceState state) {
            ...
            if (state == BleLink.DeviceState.DEVICE_STATE_LINKED) {
                handler.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        if (PlayDrumActivityBle.this.bleLink.sendData(UartDataDeal.sendCommand(0x01, 0x0B, 0x00))) {
                            MyUtil.showToast(PlayDrumActivityBle.this, "陀螺仪已打开");
                            handler.postDelayed(new Runnable() {
                                                    @Override
                                                    public void run() {
                                                        if (PlayDrumActivityBle.this.bleLink.sendData(UartDataDeal.sendCommand(0x01, 0x0B, 0x01))) {
                                                            MyUtil.showToast(PlayDrumActivityBle.this, "选择乐器");
                                                            handler.postDelayed(new Runnable() {
                                                                @Override
                                                                public void run() {
                                                                    if (PlayDrumActivityBle.this.bleLink.sendData(UartDataDeal.sendCommand(0x01, 0x00, 'D')))//选择乐器鼓
                                                                        MyUtil.showToast(PlayDrumActivityBle.this, "选择架子鼓");
                                                                }
                                                            }, 500);
                                                        }
                                                    }
